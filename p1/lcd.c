...
// E pulse timing only needs > ~1 us; we'll be conservative.
static void lcd_delay_short(void)
{
    // A few microseconds worth of NOPs (rough, but fine for LCD)
...
{
    // Put value on PC0..PC7 without disturbing PC8/PC9
...
static void lcd_write(uint8_t value, uint8_t rs)
{
    ...
    // Conservative instruction delay: most writes are fast,
    // but safe to wait ~1ms during bring-up.
    HAL_Delay(1);
}
...
{
    // HD44780 typical DDRAM addresses:
    // Row 0: 0x00 + col
    // Row 1: 0x40 + col
...
void LCD_init(void)
{
    ...
    // Push-pull, no pull-ups, medium speed is fine
    GPIOC->OTYPER &= ~(
    ...
    LCD_command(0x0F);  // Display ON, cursor ON, blink ON (gives visible cursor)
    LCD_command(0x01);  // Clear display
    HAL_Delay(2);       // Clear needs longer
